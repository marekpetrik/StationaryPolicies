...
structure Findist (Ω : List τ) : Type where
  pr : List ℝ                      -- probability measure 
  gezero : ∀p ∈ pr, 0 ≤ p          -- separate for convenience
  sumsto : pr.sum = 1              -- sums to 1
  unique : List.Nodup Ω            -- the elements of Ω are unique
  length : pr.length = Ω.length    -- lengths are the same
...
def Findist.append {Ω : List τ} (F : Findist Ω) (ω : τ) (p : ℝ) (gezero : p ∈ I) :=
    let pr' := pr.map (fun p' ↦ p' / (1-p))
    sorry

#check Finprob.rec

def Finprob.ind {α : Type*} {motive : List α → Sort*} (nil : motive [])
