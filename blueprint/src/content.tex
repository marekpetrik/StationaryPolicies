...
A \emph{finite probability measure} $p\colon \Omega \to \Real_+$ for a finite set $\Omega$ is any function that satisfies
\[
...
A \emph{finite probability space} is $P = (\Omega, \Delta(\Omega))$, where $\Omega$ is a finite set and the $\sigma$-algebra is $2^{\Omega}$.
\lean{Finprob} \leanok
\end{definition}
...
\begin{definition}
The \emph{expectation} of a random variable $\tilde{x} \colon \Omega \to \Real$ defined on a probability space $(\Omega, p)$ is defined as
\[
...
\begin{definition}
An \emph{indicator} function $\I \colon \mathcal{B} \to \left\{ 0, 1 \right\}$ is defined as
\[
\I(a) :=
\begin{cases}
...
\end{cases}
\]
\lean{Finprob.𝕀} \leanok
\end{definition}

\begin{definition}
The \emph{probability} of $\tilde{b}\colon \Omega \to \mathcal{B}$ on a probability space $P = (\Omega, p)$ is defined as
\[
...
\begin{definition}
The \emph{conditional expectation} of a random variable $\tilde{x} \colon \Omega \to \Real$ given an indicator $\tilde{b} \colon \Omega \to  \left\{ 0, 1  \right\}$ defined on a probability space $(\Omega, p)$ is defined as
\[
\E \left[ \tilde{x} \mid  \tilde{c} \right] := \frac{1}{\P[\tilde{c}]} \sum_{\omega \in \Omega } p(\omega ) \cdot \tilde{x}(\omega) \cdot \tilde{b}(\omega),
\]
...
:=
\frac{1}{\P[\tilde{y} = \tilde{y}(\omega)]} \sum_{\omega' \in \Omega } p(\omega) \cdot \tilde{x}(\omega') \cdot \I\left( \tilde{y}(\omega') = \tilde{y}(\omega) \right), \quad \forall \omega \in \Omega.
\]
where $x / 0 = 0$ for each $x\in \Real$ (see \texttt{div\_zero}).
