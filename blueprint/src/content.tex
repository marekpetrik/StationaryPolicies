...
The \emph{history-dependent policy Bellman operator} $L_{\mathrm{h}}^{\pi} \colon \mathcal{U} \to \mathcal{U}$ is defined for $\pi\in \PiHR$ as
  \[
    ...
    :=
    \E^{h, \pi, 1} \left[ \rew_{l_{\mathrm{h}}(h)} + \tilde{u} \right], \quad \forall \tilde{u} \in \mathcal{U},
  \]
...
The \emph{history-dependent optimal Bellman operator} $L_{\mathrm{h}}\opt \colon \mathcal{U} \to \mathcal{U}$ is defined as
  \[
    ...
    :=
    \max_{a\in \mathcal{A}} \E^{h, a, 1} \left[ \rew_{l_{\mathrm{h}}(h)} + \tilde{u} \right], \quad \forall \tilde{u} \in \mathcal{U},
  \]
  where the value function $\tilde{u}$ is interpreted as a random variable on defined on the sample space $\Omega = \mathcal{H}$.
...
% \begin{align*}
% \hat{u}_{t+1}(h; \pi)
% &= \E^{h, \pi, t+1} \left[ \rew \right] -\rew(h) && \why{\cref{def:u-pi}}\\
% &= \E^{h, \pi, t+1} \left[ \sum_{k=0}^{k_0+t}  r(\tilde{s}_k, \tilde{a}_k, \tilde{s}_{k+1})  \right] -\rew(h) && \why{\cref{thm:expret-eq-sum-rew}}\\
% &= \E^{h, \pi, t+1} \left[ \sum_{k=k_0}^{k_0+t}  r(\tilde{s}_k, \tilde{a}_k, \tilde{s}_{k+1})  \right]  && \why{\cref{thm:sum-rew-eq-sum-rew-rg}} \\
% &= \E^{h, \pi, t+1} \left[  \E^{h, \pi, t+1} \left[ \sum_{k=k_0}^{k_0+t} r(\tilde{s}_k, \tilde{a}_k, \tilde{s}_{k+1}) \mid \tilde{a}_{k_0}, \tilde{s}_{k_0+1} \right] \right]  && \why{\cref{thm:total-expectation-h}} \\
% &= \E^{h, \pi, t+1} \left[\tilde{r}_{k_0} +  \E^{h, \pi, t+1} \left[ \sum_{k=k_0+1}^{k_0+t}  r(\tilde{s}_k, \tilde{a}_k, \tilde{s}_{k+1}) \mid \tilde{a}_{k_0}, \tilde{s}_{k_0+1} \right] \right] && \why{\cref{thm:sum-rew-cnd}} \\
% &= \E^{h, \pi, t+1} \left[\tilde{r}_{k_0} +  \E^{\langle h,\tilde{a}_{k_0},\tilde{s}_{k_0+1}\rangle, \pi, t} \left[ \sum_{k=k_0+1}^{k_0+t} r(\tilde{s}_k, \tilde{a}_k, \tilde{s}_{k+1})  \right] \right] && \why{\cref{thm:exph-cond-eq-hist}} \\
% &= \E^{h, \pi, t+1} \left[\tilde{r}_{k_0}  +  \E^{\langle h,\tilde{a}_{k_0},\tilde{s}_{k_0+1}\rangle, \pi, t} \left[ \sum_{k=0}^{k_0+t} r(\tilde{s}_k, \tilde{a}_k, \tilde{s}_{k+1})  \right] - \tilde{r}_{k_0+1} \right] &&  \why{\cref{thm:sum-rew-eq-sum-rew-rg}}\\
% &= \E^{h, \pi, t+1} \left[\tilde{r}_{k_0}+  \E^{\langle h,\tilde{a}_{k_0},\tilde{s}_{k_0+1}\rangle, \pi, t} \left[ \rew \right] - \rew(\langle h, \tilde{a}_{k_0}, s_{k_0+1}\rangle) \right] &&  \why{\cref{thm:expret-eq-sum-rew}}\\
% &= \E^{h, \pi, t+1} \left[\tilde{r}_{k_0} +  \hat{u}_t(\langle h,\tilde{a}_{k_0},\tilde{s}_{k_0+1}\rangle) \right] && \why{\cref{def:u-pi}} \\
% &= \E^{h, \pi, t+1} \left[\tilde{r}_{k_0} +  u^{\pi}_t(\langle h,\tilde{a}_{k_0},\tilde{s}_{k_0+1}\rangle) \right] && \why{inductive assm} \\
% &= \E^{h, \pi, 1} \left[\rew +  \tilde{u}^{\pi}_t \right] && \why{???} \\
% &= L_{\mathrm{h}}^{\pi} u_t^{\pi} && \why{\cref{def:DPhpi}} \\
% &= u^{\pi}_t(h). && \why{\cref{def:u-dp-pi}}
% \end{align*}
% }
...
\begin{align*}
  u\opt_{t+1}(h)
  &= \max_{a\in \mathcal{A}} \E^{h, a, 1} \left[ \rew_{l_{\mathrm{h}}(h)} + \tilde{u}\opt_t  \right]
  && \why{\cref{def:u-dp-opt}} \\
  &= \max_{a\in \mathcal{A}} \E^{h, a, 1} \left[ \rew_{l_{\mathrm{h}}(h)} + v\opt_t(\tilde{s}_l) \right] 
    && \why{inductive asm.}  \\
  &= \max_{a\in \mathcal{A}} \E^{s_0, a, 1} \left[ \rew_{l_{\mathrm{h}}(h)} + v\opt_t(\tilde{s}_l) \right]
...
\[
  v\opt_{T-k}(s_{\mathrm{l}}(h)) = u^{\pi\opt}_{T-k}(h), \quad
  \forall h\in \left\{ h\in \mathcal{H} \mid l_{\mathrm{h}}(h) \le T \right\},
\]
$k := l_{\mathrm{h}}(h)$.
\lean{MDPs.v_dp_opt_eq_u_opt} 
\end{theorem}
...
\end{align*}
Here, $k := {l_{\mathrm{h}}(h)}$ $a\opt := \pi\opt_T(k, s_0)$ and $s_0 := s_{\mathrm{l}}(h)$
\end{proof}
