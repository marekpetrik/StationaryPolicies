...
Suppose that $\tilde{x}, \tilde{y}\colon \Omega \to \Real$ are random variables and $c\in \Real$, such that $\tilde{y}(\omega) = c + \tilde{x}(\omega)$. Then
\[
...
\begin{theorem} \label{thm:exp-cnd-rv-add-cons}
...
The \emph{history-dependent policy Bellman operator} $\mathcal{B}_{\mathrm{h}}^{\pi} \colon \mathcal{U} \to \mathcal{U}$ is defined for $\pi\in \PiHR$ as
  \[
    ...
    \sum_{a\in \mathcal{A}} \pi(h,a) \cdot  \sum_{s'\in \mathcal{S}} P(l_{\mathrm{s}}(h), a, s') \cdot
...
The \emph{history-dependent optimal Bellman operator} $\mathcal{B}_{\mathrm{h}}\opt \colon \mathcal{U} \to \mathcal{U}$ is defined for $\pi\in \PiHR$ as
  \[
    ...
    \max_{a\in \mathcal{A}} \sum_{s'\in \mathcal{S}}   P(l_{\mathrm{s}}(h), a, s') \cdot u \left( \langle h, a, s' \rangle \right)
  \]
  \lean{DPhopt}\leanok
\end{definition}
...
\begin{definition} \label{def:u-dp-pi}
  ...
  \[
   ...
   \begin{cases}
     0 &\text{if } t = 0, \\
     \mathcal{B}_{\mathrm{h}}^{\pi} u_{t-1}^{\pi} &\text{otherwise}.
   \end{cases}
 \]
 \lean{u_dp_π}\leanok
\end{definition}
...
\begin{definition} \label{def:u-dp-opt}
  ...
  \[
   ...
   \begin{cases}
     0 &\text{if } t = 0, \\
     \mathcal{B}_{\mathrm{h}}\opt u_{t-1}\opt &\text{otherwise}.
   \end{cases}
 \]
...
\begin{lemma}\label{thm:dp-opt-ge-dp-pi}
  ...
  \[
   (\mathcal{B}_{\mathrm{h}}\opt u^1)(h) \ge  
   (\mathcal{B}_{\mathrm{h}}^{\pi} u^2)(h), \quad \forall \pi\in \PiHR, h\in \mathcal{H}.
  \]
\end{lemma}
\lean{dp_opt_ge_dp_pi}
\begin{proof}
  For each $h\in \mathcal{H}, \pi \in \PiHR$:
\begin{align*}
(\mathcal{B}_{\mathrm{h}}\opt u^1)(h)
&= \max_{a\in \mathcal{A}} \sum_{s'\in \mathcal{S}} P(l_{\mathrm{s}}(h), a, s') \cdot u^1 \left( \langle h, a, s' \rangle \right) \\
&\ge \sum_{a\in \mathcal{A}} \pi(h,a) \cdot  \sum_{s'\in \mathcal{S}} P(l_{\mathrm{s}}(h), a, s') \cdot u^1 \left( \langle h, a, s' \rangle \right) \\
&\ge \sum_{a\in \mathcal{A}} \pi(h,a) \cdot  \sum_{s'\in \mathcal{S}} P(l_{\mathrm{s}}(h), a, s') \cdot u^2 \left( \langle h, a, s' \rangle \right).
\end{align*}
This follows because the probabilities sum to 1 and are non-negative. 
\end{proof}

The following theorem shows the history-dependent value function equals to the history-dependent value function computed using a dynamic program.
...
&= \E^{h, \pi, t+1} \left[ \sum_{k=0}^{k_0+t}  r(\tilde{s}_k, \tilde{a}_k, \tilde{s}_{k+1})  \right] -\tilde{r}_{\mathrm{h}}(h) && \text{\cref{thm:expret-eq-sum-rew}}\\
...
&= \E^{h, \pi, t+1} \left[\tilde{r}_{k_0} +  \hat{u}_t(\langle h,\tilde{a}_{k_0},\tilde{s}_{k_0+1}\rangle; \pi) \right] && \text{\cref{def:u-pi}} \\
&= \E^{h, \pi, t+1} \left[\tilde{r}_{k_0} +  u^{\pi}_t(\langle h,\tilde{a}_{k_0},\tilde{s}_{k_0+1}\rangle; \pi) \right] && \text{inductive assm} \\
&= u^{\pi}_t(h). && \text{\cref{def:DPhpi,def:u-dp-pi}}
\end{align*}
}
Here, we use $\tilde{r}_{k_0} := r(\tilde{s}_{k_0}, \tilde{a}_{k_0}, \tilde{s}_{k_0+1})$ and $\tilde{r}_{k_0+1} = \tilde{r}_{\mathrm{h}}(\langle h, \tilde{a}_{k_0}, s_{k_0+1}\rangle)$.
\end{proof}
...
\begin{proof}
  ...
  \begin{align*}
    u_{t+1}\opt(h)
    &= (\mathcal{B}_{\mathrm{h}}\opt u_t\opt)(h) && \text{\cref{def:DPhopt}} \\
    &\ge (\mathcal{B}_{\mathrm{h}}^{\pi} \hat{u}_t(\cdot; \pi))(h) && \text{ind asm, \cref{thm:dp-opt-ge-dp-pi}} \\
    &= (\mathcal{B}_{\mathrm{h}}^{\pi} u_t^{\pi})(h) && \text{\cref{thm:dph-correct-vf}} \\
