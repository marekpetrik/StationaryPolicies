...
In the remainder of this section we assume an MDP $M = (\mathcal{S}, \mathcal{A}, p, r)$.
...
\begin{definition}
The set of \emph{non-empty histories} is
\[
...
\[
\Pi_{\mathrm{HR}} := \mathcal{S} \to \Delta(\mathcal{A}).
\]
\lean{PolicyHR} \leanok
\end{definition}

\begin{definition}
The probability distribution, define in \cref{def:probability-space}, $p^{\mathrm{h}}_T \in \Delta(\mathcal{H}_T(\hat{h}))$ over histories $\mathcal{H}_T(\hat{h})$ for an MDP $M = (\mathcal{S}, \mathcal{A}, p, r)$ and $\pi \in \Pi_{\mathrm{HR}}$ is defined for each $T \in \Nats$ and $h\in \mathcal{H}_T(\hat{h})$ as
\[
p^{\mathrm{h}}_T(h) :=
\begin{cases}
